...
class WindowRenderer(Renderer):
    ...
    def render(self, game_state: GameState) -> None:  # pragma: no cover (UI)
...
class WindowHumanPlayer(Player):
    ...
    def get_move(self, game_state: GameState):  # pragma: no cover (UI)
        # Wait for a click callback to set pending index.
...
@dataclass
class GameView:  # pragma: no cover (UI)
    root: tk.Tk
